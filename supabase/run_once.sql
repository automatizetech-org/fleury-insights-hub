-- =============================================================================
-- FLEURY INSIGHTS HUB — RODE ESTE ARQUIVO UMA ÚNICA VEZ NO SUPABASE
-- =============================================================================
-- Onde: Supabase Dashboard > SQL Editor > New query > colar este conteúdo > Run
-- Este arquivo cria todo o schema (profiles, companies, company_memberships,
-- tabelas de negócio) e ativa RLS em todas as tabelas, alinhado ao app.
-- Após rodar: crie o usuário admin (Edge Function create-admin ou pelo Dashboard).
-- =============================================================================
...
-- 5) Trigger: ao inserir company, inserir membership owner para created_by e todos os demais como member
...
begin
  ...
  -- Nova empresa fica disponível para todos os usuários
  insert into public.company_memberships (company_id, user_id, member_role)
...
-- 5b) Trigger: ao criar perfil (novo usuário), adicioná-lo como membro de todas as empresas
...
-- 6) Função auxiliar RLS: company_ids do usuário (usada pelas policies)
...
-- companies: qualquer autenticado pode inserir; select para todos autenticados; update/delete só se for membro ou super_admin
...
-- company_memberships: select se membro da empresa ou super_admin; insert para owner/super_admin
...
-- 10) RLS nas tabelas de negócio (usuário só vê dados das empresas em que é membro; super_admin vê tudo)
...
-- 11) Trigger: criar profile ao inserir em auth.users (cada novo usuário ganha linha em profiles)
...
-- =============================================================================
-- Fim. Para criar o admin (login: admin, senha: admin123):
-- Ver instruções detalhadas em: supabase/CRIAR_ADMIN.md
--
-- Resumo rápido (Dashboard):
-- 1) Authentication > Users > Add user: email admin@fleury-insights.local, senha admin123
-- 2) Copie o User UID e rode:
--    update public.profiles set username = 'admin', role = 'super_admin' where id = 'COLE_O_UUID_AQUI';
-- 3) Para o login funcionar, publique a Edge Function: npx supabase functions deploy auth
-- =============================================================================
